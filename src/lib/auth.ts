...
export function updateAvatar(userId: string, avatarUrl: string): User {
  ...
  try {
    ...
    throw new Error("บันทึกรูปโปรไฟล์ไม่สำเร็จ");
  }
...
export function awardPoints(
  ...
  if (index === -1) {
    if (fallbackUser) {
      return { ...fallbackUser, points: (fallbackUser.points ?? 0) + points };
    }
    throw new Error("ไม่พบบัญชีสำหรับเพิ่มแต้ม");
  }

  accounts[index] = {
    ...accounts[index],
    points: (accounts[index].points ?? 0) + points,
  };
  try {
    localStorage.setItem(ACCOUNTS_KEY, JSON.stringify(accounts));
  } catch {
    throw new Error("บันทึกแต้มไม่สำเร็จ");
  }
  return publicUser(accounts[index]);
}
...
export function deductPoints(
  ...
  if (index === -1) {
    if (fallbackUser) {
      const current = fallbackUser.points ?? 0;
      if (current < points) throw new Error("แต้มสะสมไม่เพียงพอ");
      return { ...fallbackUser, points: current - points };
    }
    throw new Error("ไม่พบบัญชีสำหรับหักแต้ม");
  }

  const current = accounts[index].points ?? 0;
  if (current < points) throw new Error("แต้มสะสมไม่เพียงพอ");
  accounts[index] = { ...accounts[index], points: current - points };
  try {
    localStorage.setItem(ACCOUNTS_KEY, JSON.stringify(accounts));
  } catch {
    throw new Error("บันทึกแต้มไม่สำเร็จ");
...
export async function registerAccount(
  input: { studentId: string; email: string; phone: string; password: string },
) {
  ...
  if (!/^0\d{8,9}$/.test(phone)) {
    throw new Error("กรุณาระบุเบอร์โทรศัพท์ 9 หรือ 10 หลัก");
  }
  ...
  try {
    ...
    throw new Error("บันทึกบัญชีไม่สำเร็จ กรุณาตรวจสอบพื้นที่จัดเก็บข้อมูล");
  }
...
export async function loginAccount(emailInput: string, password: string) {
  const account = readAccounts().find((item) =>
    item.email === emailInput.trim().toLowerCase()
  );
  if (
    !account ||
